########################################################
# AbASA_18.6A_calculations_in_parts_plot.py is a script 
# that will plot AbASA values (18.6 Angstrom probe) from 
...
# using the command:
# >python AbASA_18.6A_calculations_in_parts_plot.py
#
# This script requires that you have already run:
#   AbASA_18.6A_calculations_in_parts1.py and
#   AbASA_18.6A_calculations_in_parts2.py
#
# The following files will be generated:
#   AbASA_18.6A_CV3-CC25_per_ensemble.png
#   AbASA_18.6A_CV3-CC25_per_ensemble.svg
#   AbASA_18.6A_woglycans_CV3-CC25_per_ensemble.png
#   AbASA_18.6A_woglycans_CV3-CC25_per_ensemble.svg
#   AbASA_18.6A_time_sampled_by_each_ensemble.png
#   AbASA_18.6A_time_sampled_by_each_ensemble.svg
#   AbASA_18.6A_times-summary.txt
#
########################################################
...
def plotSingleRun():
    ...
    sasa = md.shrake_rupley(trj_all,probe_radius=1.86,mode='residue')
    ...
    plt.xlabel('Time (us)')
    plt.savefig('AbASA_18.6A_epitopes.svg')
    plt.savefig('AbASA_18.6A_epitopes.png')
    ...
    fig.colorbar(max, cax=cbar_ax, label='AbASA (nm^2)')
    plt.savefig('AbASA_18.6A_single_trajectory.png')
    plt.savefig('AbASA_18.6A_single_trajectory.svg')
...
def getFramesInEnsembles(runID,ensemble=None):
    ...
    if ensemble ==  None:
        ...
        print("Analyzing ensemble \"{}\" for {}".format(ensemble,XTC))
    ...
    try:
        ...
        if ensemble == None:
            # Count the number of frames the pass filter based on TM-HG-HR1 angle, HG-HR1 distance and HR1-TM distance
            ...
            *(EnsembleDefs[ensemble][2][0]<dist_TM_HR1)*(EnsembleDefs[ensemble][2][1]>dist_TM_HR1)
            ...
            #Return frames of the trajectoy that pass through the filtering step of the 'k' Cryo-ET density
...
def plotTimes():
    ...
    outFile = open('AbASA_18.6A_time-summary.txt', 'w')
    ...
    plt.tight_layout()
    plt.savefig('AbASA_18.6A_time_sampled_by_each_ensemble.png')
    plt.savefig('AbASA_18.6A_time_sampled_by_each_ensemble.svg')

def getSingleSASA(trj):
    sasa = md.shrake_rupley(trj,probe_radius=1.86,mode='residue')
    psasa=sasa[:,:1584].reshape((len(sasa),3,-1))
...
# we are just plotting previous analysis.  So, no need to calculate SASA again
#    dens_entries=[]
#    dens_entries_p=[]
#    for k in [1,2,3]:
#        ddict   = []
#        ddict_p = []
#        # figure out which trajectories to analyze using this rank
#        runstoanalyze=np.where(results[:,k]>0)[0]
#        runstoanalyze=runstoanalyze[runstoanalyze>=rank*THREADS]
#        runstoanalyze=runstoanalyze[runstoanalyze<(rank+1)*THREADS]
#        # run the core SASA evaluations
#        outputs = Parallel(n_jobs=THREADS)(delayed(distributedSASA)(xtcs[m],k) for m in runstoanalyze)
...
#    with open("AbASA_18.6A_step2data."+str(rank)+".pkl", "wb") as fp: 
#        pickle.dump(dens_entries, fp)
#    with open("AbASA_18.6A_step2pdata."+str(rank)+".pkl", "wb") as fp: 
#        pickle.dump(dens_entries_p, fp)

# read in all pkl files, aggregate the statistics and then plot.
    dens_entries=[np.array([]),np.array([]),np.array([]),np.array([]),np.array([]),np.array([])]
    dens_entries_p=[np.array([]),np.array([]),np.array([]),np.array([]),np.array([]),np.array([])]
    for i in np.arange(12):
        with open("AbASA_18.6A_step2data."+str(i)+".pkl", "rb") as fp: 
            inputs=pickle.load(fp)
        for j in np.arange(6):
            dens_entries[j]=np.concatenate((dens_entries[j],inputs[j]))
        with open("AbASA_18.6A_step2pdata."+str(i)+".pkl", "rb") as fp: 
            inputs=pickle.load(fp)
        for j in np.arange(6):
            dens_entries_p[j]=np.concatenate((dens_entries_p[j],inputs[j]))
    ...
    ax.set_ylim([0,6])
    ax.set_ylabel('AbASA (nm^2)')
    plt.tight_layout()
    plt.savefig('AbASA_18.6A_CV3-CC25_per_ensemble.png')
    plt.savefig('AbASA_18.6A_CV3-CC25_per_ensemble.svg')
    ...
    ax.set_ylabel('AbASA (nm^2)')
    plt.tight_layout()
    plt.savefig('AbASA_18.6A_woglycans_CV3-CC25_per_ensemble.png')
    plt.savefig('AbASA_18.6A_woglycans_CV3-CC25_per_ensemble.svg')
...
# SingleRun was called using first scipt.
...
# Step 1 done already.  Going to read the part1.pkl file to continue
...
#with open("AbASA_18.6A_step1data.pkl", "wb") as fp: 
#    pickle.dump(outputs, fp)
with open("AbASA_18.6A_step1data.pkl", "rb") as fp: 
