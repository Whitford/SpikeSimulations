...
static int assign_param(t_functype ftype, t_iparams* newparam, gmx::ArrayRef<const real> old, int comb, double reppow)
{
    ...
    switch (ftype)
    {
        ...
        case F_POSRES:
            ...
            break;
	 /* ESTEBAN */
	 /* ADDED SIGMA for Gaussian FBPOS */
        case F_FBPOSRES:
            ...
            newparam->fbposres.sigma    = old[3];
            newparam->fbposres.RR       = old[4];
            newparam->fbposres.pos0[XX] = old[5];
            newparam->fbposres.pos0[YY] = old[6];
            newparam->fbposres.pos0[ZZ] = old[7];
            break;
        case F_DISRES:
