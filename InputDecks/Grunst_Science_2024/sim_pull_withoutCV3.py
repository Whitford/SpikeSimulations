...
def add_extra_forces(sbm):
    print('***************************************************************')
    print('Loading other forces (TM and HD forces)')
    ...
    #Find heavy atoms
    gro = GromacsGroFile("./sim_without_CV3.gro")
    ref =  md.load("./sim_without_CV3.gro")
    ...
    for index in heavy:
        if index < 15208:
            ...
            print('Pos res not applied to: ',index)
...
sbm_grofile = './sim_without_CV3.gro'
sbm_topfile = './sim_without_CV3.top'
sbm_xmlfile = './sim_without_CV3.xml'

sbm.loadSystem(Grofile=sbm_grofile, Topfile=sbm_topfile, Xmlfile=sbm_xmlfile)

#Remove pseudoparticle contacts/interactions
f=sbm.system.getForce(8)
print("Energy force 8:",f.getEnergyFunction(),f.getName())
f.setEnergyFunction("0")
print("Changing 8 to",f.getEnergyFunction(),f.getName())
f=sbm.system.getForce(9)
print("Energy force 9:",f.getEnergyFunction(),f.getName())
f.setEnergyFunction("0")
print("Changing 9 to",f.getEnergyFunction(),f.getName())
...
#Short simulation of Spike with Position restraints  
# this is only here so that the initial equilibration interval is similar
# to the no-Ab system
secs=10
...
for m in range(100):
    if dist_HG_HR1==0:
        ...
        trj_all=md.load(sbm.folder+'/trajectory.xtc',top='./sim_without_CV3.gro')
