...
/* compute flat-bottomed positions restraints */
{
    ...
    const t_iparams* pr;
    real             kk, v, ssigma, RR;
    real             dr, dr2, rfb, rfb2, fact, rfb1, rfb12, rfb22;
    ...
    for (i = 0; (i < nbonds);)
    {
        ...
        switch (pr->fbposres.geom)
        {
            ...
            case efbposresZ: /* fbdim=ZZ */
               ...
               break;
		/* ESTEBAN */
            case efbposresGX: /* fbdim=XX */
            case efbposresGY: /* fbdim=YY */
            case efbposresGZ: /* fbdim=ZZ */
                /* 1D Gaussian flat-bottom potential */
                fbdim = pr->fbposres.geom - efbposresGX;
                dr    = dx[fbdim];
                if ((dr > rfb && !bInvert))
                {
                    v         = -kk * std::exp( -0.5 * gmx::square( (dr - rfb ) / ssigma ) ) + kk ;
                    fm[fbdim] = ( kk * ( dr - rfb ) * std::exp(- 0.5 * gmx::square( ( dr - rfb ) / ssigma ) ) ) / gmx::square(ssigma) ;
                }

                else if ((dr < -rfb && !bInvert))
                {
                    v         = -kk * std::exp( -0.5 * gmx::square( (dr + rfb ) / ssigma ) ) + kk ;
                    fm[fbdim] = ( kk * ( dr + rfb ) * std::exp(- 0.5 * gmx::square( ( dr + rfb ) / ssigma ) ) ) / gmx::square(ssigma);
                }
		...
		rfb1 = RR-rfb;
        	rfb2 = RR+rfb;
        	rfb12 = gmx::square(RR-rfb);
        	rfb22 = gmx::square(RR+rfb);
                if ( ( dr2 < rfb12 && !bInvert ) )
                {
                    dr   = std::sqrt(dr2);
                    v    = -kk * std::exp( -0.5 * gmx::square( (dr - rfb1 ) / ssigma ) ) + kk ;
                    fact =  ( kk * ( dr - rfb1 ) * std::exp(- 0.5 * gmx::square( ( dr - rfb1 ) / ssigma ) ) ) / (gmx::square(ssigma) * dr);
                    svmul(fact, dx, fm);
                }
		else if ( ( dr2 > rfb22  && !bInvert ) )
                 {
                    dr   = std::sqrt(dr2);
                    v    = -kk * std::exp( -0.5 * gmx::square( (dr - rfb2 ) / ssigma ) ) + kk ;
                    fact =  ( kk * ( dr - rfb2 ) * std::exp(- 0.5 * gmx::square( ( dr - rfb2 ) / ssigma ) ) ) / (gmx::square(ssigma) * dr);
                    svmul(fact, dx, fm);
                }
               break;
